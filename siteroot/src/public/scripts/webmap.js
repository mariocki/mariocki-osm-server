...
var Leaf = require("./L.KML.js");
var leaflet_geosearch_1 = require("leaflet-geosearch");
var provider = new leaflet_geosearch_1.BingProvider({
    params: {
        key: process.env.BING_KEY
    }
});
var map = Leaf.map('map').setView([52, 0], 5);
map.addControl(leaflet_geosearch_1.GeoSearchControl({ provider: provider, showMarker: false, autoClose: true }));
// Base Layers
...
var AzureMaps_Imagery = L.tileLayer("https://atlas.microsoft.com/map/tile?api-version={apiVersion}&tilesetId=microsoft.imagery&x={x}&y={y}&zoom={z}&language={language}&subscription-key={subscriptionKey}", {
...
var AzureMaps_MicrosoftWeatherRadarMain = L.tileLayer("https://atlas.microsoft.com/map/tile?api-version={apiVersion}&tilesetId=microsoft.weather.radar.main&x={x}&y={y}&zoom={z}&language={language}&subscription-key={subscriptionKey}", {
    attribution: "See https://docs.microsoft.com/en-US/rest/api/maps/renderv2/getmaptilepreview#uri-parameters for details.",
    apiVersion: "2.0",
    subscriptionKey: process.env.AZURE_MAP_KEY,
    language: "en-US"
...
map.on("moveend", function () {
    ...
    document.getElementById("latlon").innerHTML = latlon.lat.toFixed(4) + ", " + latlon.lng.toFixed(4);
});
document
