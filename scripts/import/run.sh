...
import_map_data() {
    ...
    # import osm file to gis
    osm2pgsql -H ${PGHOST} -d ${GIS_DB} -U ${POSTGRES_USER} --slim -G --hstore --tag-transform-script /openstreetmap-carto/openstreetmap-carto.lua --number-processes ${THREADS:-4} -S /openstreetmap-carto/openstreetmap-carto.style /data/data.osm.pbf --create
